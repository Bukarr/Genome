...
export function SettingsView({ onReset }: SettingsViewProps) {
  ...
  const handleExportAnalyticsJSON = () => {
    ...
    downloadAnchor.setAttribute('download', `genome-user-analytics.json`);
  ...
  const handleFullReset = () => {
    ...
    clearSuggestions();
    clearCalendar();
    localStorage.removeItem('genome-chat-history');
    setIsConfirmingFullReset(false);
    toast.success('Profile wiped. Restarting Genome setup...');
        ...
        <div className="space-y-2">
          ...
          {events.length === 0 ? (
            <div className="text-center py-6 border border-dashed border-border-accent/30 rounded-xl bg-bg/25">
              ...
              <p className="text-xs text-muted font-mono leading-relaxed px-4">No actions logged yet. Go explore suggestions, chat with Genome AI, or trigger scheduler planner grids to view event logs!</p>
            </div>
          ) : (
